    ...
    case 0x00:
      switch(funct) {
        ...
        case 0x10: D(cout << "mfhi " << regNames[rd]);
                   writeDest = true; destReg = rd; stats.registerDest(rd);
                   aluOp = ADD;
                   aluSrc1 = regFile[REG_ZERO]; stats.registerSrc(REG_ZERO);
                   aluSrc2 = hi; stats.registerSrc(REG_HILO);
                   break;
        case 0x12: D(cout << "mflo " << regNames[rd]);
                   writeDest = true; destReg = rd; stats.registerDest(rd);
                   aluOp = ADD;
                   aluSrc1 = regFile[REG_ZERO]; stats.registerSrc(REG_ZERO);
                   aluSrc2 = lo; stats.registerSrc(REG_HILO);
                   break;
...
void CPU::printFinalStats() {
  ...
  cout << endl;
  cout << "Mem ops: "  << fixed << setprecision(1) << (float)stats.getMemOps() / instructions * 100 << "% of instructions" << endl;
  cout << "Branches: " << fixed << setprecision(1) << (float)stats.getBranches() / instructions * 100 << "% of instructions" << endl;
  cout << "  % Taken: " << fixed << setprecision(1) << (float)stats.getTaken() / stats.getBranches() * 100 << endl;
}
