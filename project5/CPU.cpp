...
void CPU::mem() { //aluOut contains address
int numStalls = 0; /*STALL FUNCTION NEEDS FIXED*/
  if(opIsLoad)
  {
    numStalls = cacheStats.access(aluOut, LOAD);
  ...
  if(opIsStore)
  {
    numStalls = cacheStats.access(aluOut, STORE);
    dMem.storeWord(storeData, aluOut);
  }
  stats.stall(numStalls);
}
